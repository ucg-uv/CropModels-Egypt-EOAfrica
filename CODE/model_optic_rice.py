...
for year in range(2016, 2023):
    ...
    for parcel in range(1, 150):
        ...
        if (str(parcel) + '.csv' in optic_list):
            ...
            current_optic = current_optic_1[current_optic_1['CROPTYPE'] == 'rice']

            B4 = current_optic['B4'].values
            B5 = current_optic['B5'].values
            B8 = current_optic['B8'].values

            if len(current_optic) > 0:

                for gdd in range(0, len(current_optic.GDD)):
                    calculated_yield.append(193004.78*B4[gdd] - 148414.10*B5[gdd] + 16705.91*B8[gdd] + 6111.25)
    ...
    if len(all_calculated_yield) > 0:
        ...
        for value in range(0, len(sorted_all_rmse)):
            if stop == 0:
                ...
                if 500 <= all_current_gdd[0][gdd_index] <= 1800:
                #if True:
...
plt.title('CROP: rice - All Seasons')

# Show the plot

plt.tight_layout()
#plt.savefig(os.path.join(output_path, 'all_rice.png'), bbox_inches='tight')
plt.close()
...
for season in range(0, len(all_Y)):
    ...
    for value in range(0, len(current_season)):
        if current_season_ref[value] == 9523:
            group_1.append(current_season[value])
        if current_season_ref[value] == 9761:
            group_2.append(current_season[value])
        if current_season_ref[value] == 10000:
            group_3.append(current_season[value])
        if current_season_ref[value] == 10238:
            group_4.append(current_season[value])
        if current_season_ref[value] == 10476:
...
ref_data = [9523,9523,9761,9761,9761,9761,9761,10000,10000,10000,10000,10000,10238,10238,10238,10238,10476]
...
ref_data = [9523, 9761, 10000,10238, 10476]
